    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                var model = Train(mlContext, _trainDataPath);  // Train on training data
                Evaluate(mlContext, model, _testDataPath);     // Evaluate on testing data
                TestSinglePrediction(mlContext, model);        // Test a single prediction
            }
            catch (Exception ex)
            {
        ...
        static ITransformer Train(MLContext mlContext, string dataPath)
        {
            ...
            var pipeline = mlContext.Transforms.Categorical.OneHotEncoding("GenderType") // One-hot encode GenderType
                .Append(mlContext.Transforms.Concatenate("Features", "AgeCount", "PrepurchaseAmount", "GenderType")) // Concatenate features
